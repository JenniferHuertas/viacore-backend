        ...
        if (trainingRequest) {
          ...
          if (askingForStatus) {
            ...
            contextForAi +=
              ...
              `El usuario está preguntando específicamente ` +
              `por el estado de su solicitud actual. ` +
              `Debes responder PRIMERO el estado real ` +
              `utilizando exclusivamente los datos reales ` +
              `de la base de datos antes de continuar ` +
              `con cualquier explicación comercial.`;
          }
